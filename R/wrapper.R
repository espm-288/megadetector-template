...
download_model <- function(model = "MDV5A", force = FALSE) {
  ...
  if (!model %in% names(models)) {
    cli::cli_abort(
      "Unknown model: {.val {model}}. Available models: {.val {names(models)}}"
    )
  }
  ...
  if (file.exists(dest) && !force) {
    cli::cli_alert_success(
      "Model {.val {model}} found in cache: {.path {dest}}"
    )
    return(dest)
  }
...
load_model <- function(model_file = NULL) {
  if (is.null(md)) {
    cli::cli_abort("MegaDetector module not loaded.")
  }
  ...
  rd <- tryCatch(
    ...
    error = function(e) {
      cli::cli_abort(
        "Could not import megadetector.detection.run_detector: {e$message}"
      )
    }
  )
...
detect_image <- function(model, image_file) {
  ...
  vis_utils <- tryCatch(
    ...
    error = function(e) {
      cli::cli_abort(
        "Could not import megadetector.visualization.visualization_utils: {e$message}"
      )
    }
  )
